...
void window_and_line_buffer(hls::ap_uint< 10 > input, hls::ap_uint< 10 > window[3][3]);

// Gaussian Filter.
const unsigned int GF_KERNEL_SIZE = 5;
void gaussian_filter(hls::FIFO< unsigned char > &input_fifo, hls::FIFO< unsigned char > &output_fifo);
...
void sobel_filter(hls::FIFO< unsigned char > &input_fifo, hls::FIFO< unsigned short > &output_fifo);

// Non-maximum suppression.
const unsigned int NM_KERNEL_SIZE = 3;
void nonmaximum_suppression(hls::FIFO< unsigned short > &input_fifo, hls::FIFO< unsigned char > &output_fifo);
